defmodule TestLens.JSONReportTest do
  ...
  defp failed_result do
    %Result{
      ...
      status: :failed, time_us: 200, failures: [{:error, %RuntimeError{message: "x"}, []}],
  ...
  test "build/3 with a passed and a failed result counts correctly" do
    artifact = JSONReport.build([passed_result(), failed_result()], %{run: 0, async: nil, load: nil}, nil)
  ...
  test "build/3 does not include any environment, secrets, or application config" do
    ...
    expected_keys = MapSet.new([
      "test_lens_version", "project", "timestamp", "seed", "totals",
      "times_us", "failures", "slow", "classification_counts", "next_commands"
    ])
    assert MapSet.new(Map.keys(artifact)) == expected_keys
  end

  test "build/3 classification_counts is a histogram of failure types" do
    artifact = JSONReport.build([failed_result(), failed_result()], %{run: 0, async: nil, load: nil}, nil)
  ...
  test "write/4 the file content is valid JSON", %{dir: dir} do
    path = Path.join(dir, "report.json")
    :ok = JSONReport.write(path, [passed_result(), failed_result()], %{run: 0, async: nil, load: nil}, 7)
